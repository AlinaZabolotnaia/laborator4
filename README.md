...
6. **Контрольные вопросы**
	1.	В процессе настройки веб-сервера и размещения WordPress, как правило, изменяются следующие конфигурационные файлы:

	•	apache2.conf или 000-default.conf — для настройки виртуального хоста Apache;
	•	php.ini — для настройки параметров PHP, таких как размер загружаемых файлов или лимиты выполнения;
	•	wp-config.php — основной конфигурационный файл WordPress, содержащий данные для подключения к базе данных и другие настройки.

	2.	Директива DirectoryIndex в Apache указывает имя файла, который будет автоматически загружаться при обращении к каталогу на веб-сервере, если в запросе не указан конкретный файл. Например, если в директории есть index.html, и указан DirectoryIndex index.html, то этот файл будет отображён по адресу http://example.com/. Если указано несколько имён файлов, Apache будет использовать первый найденный.
	3.	Файл wp-config.php необходим для настройки работы WordPress. Он содержит параметры подключения к базе данных (имя базы данных, имя пользователя, пароль, адрес сервера), ключи безопасности, префикс таблиц базы данных и другие системные параметры. Без корректно настроенного wp-config.php WordPress не сможет работать.
	4.	Параметр post_max_size в файле php.ini отвечает за максимальный размер данных, которые можно передать серверу в теле POST-запроса. Это влияет, например, на загрузку файлов через формы. Значение этого параметра должно быть больше или равно значению upload_max_filesize, иначе возможны ошибки при загрузке.
	5.	Возможные недостатки созданного Docker-образа:

	•	Он может быть слишком «тяжёлым», если не использовались минимальные базовые образы или если установлены ненужные пакеты;
	•	Не оптимизирован кэш Docker-слоёв, из-за чего сборка может занимать больше времени;
	•	Конфигурационные файлы и пароли могут быть захардкожены прямо в образе, что небезопасно;
	•	Отсутствует разделение на слои (например, приложение, конфигурация, данные), что усложняет обновление и поддержку;
	•	Нет настройки постоянного хранилища (volumes), поэтому все данные теряются при удалении контейнера.
